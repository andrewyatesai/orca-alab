...
/// `print` calls.
///
/// Test-only: production uses `advance_fast`/`ActionSink`. The batch path
/// (`advance_batch`) — the only consumer of this trait — is `#[cfg(test)]`,
/// so the trait and its impls are gated out of release builds.
#[cfg(test)]
