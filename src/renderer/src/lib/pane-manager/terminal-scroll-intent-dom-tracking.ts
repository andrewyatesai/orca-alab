  ...
  try {
    const dataSubscription = onData((data: string) => {
      ...
      if (typeof onUserInput === 'function') {
        ...
        // Fork (aterm): the facade has no _core.onUserInput, but its onData only
        // carries user-origin data (input()/paste; controller keystrokes route
        // through pty-connection's input seam), so every emission may resync.
