 ...
 *
 * `humanizeBranchSlug` is injected because its canonical implementation moved to
 * the Rust orca-core branch-name core, which each environment reaches through a
 * different binding (main via napi, renderer via wasm) — shared code can't pick.
 */
export function deriveWorkspaceDisplayName(input: {
  prompt: string
  slug: string
  resolvedLeaf?: string
  humanizeBranchSlug: (slug: string) => string
}): string {
  const identifier = extractWorkIdentifier(input.prompt)
  if (!identifier) {
    return input.humanizeBranchSlug(input.resolvedLeaf ?? input.slug)
  }
