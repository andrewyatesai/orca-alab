...
- **F4 Close the loop** [L]: unattended classify→port→verify→promote for in-fragment kernels **whose
  ...
  the discovery-coverage fix + soundness-control twins + the allocation-recovery pass): **88 TRUSTED /
  249 runnable / 0 declined / 6 controls refuted / 0 soundness breaks** — the numerator moved 54→88 by
