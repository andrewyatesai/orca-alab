...
export type AtermDrawStrategy = {
  ...
  /** Worker path only: parse fence — resolves TRUE once the worker engine has parsed
   *  every process() byte posted before it (its auto-replies already delivered), FALSE
   *  on the fence timeout/dispose (worker merely behind). The replay guard holds its
   *  drop window open until this resolves true. Unset for in-process strategies (their
