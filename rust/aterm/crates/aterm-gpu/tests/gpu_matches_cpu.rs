...
fn decorations_gpu_match_cpu() {
    ...
    let render = |cpu: &mut Renderer,
        ...
        term.process(bytes);
        let input = term.cell_frame(rows, cols);
        (cpu.render_input(&input), gpu.render_input(win, &input))
    };
...
\u{2764}\u{fe0f} e\u{0301} \u{1b}[4;9mUu\u{1b}[0m"
    ...
    for (i, &(rows, cols)) in dims.iter().enumerate() {
        ...
        let input = term.cell_frame(rows, cols);
        let reused_frame = reused.render_input(&mut win_reused, &input);
        let fresh_frame = fresh.render_input(&mut win_fresh, &input);
