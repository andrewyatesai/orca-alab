...
async function computeSubmoduleRangeEntries(
  ...
  const statsByPath = parseNumstatNative(numstat)
...
export async function getStagedCommitContext(
  ...
  try {
    ...
    // Why: staged patch is optional context (the Rust prompt builder truncates it anyway); degrade to file-name summary rather than fail.
    console.warn(
