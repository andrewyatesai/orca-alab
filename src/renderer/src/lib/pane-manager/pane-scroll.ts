...
import {
  ...
  readMarkerRenumberDelta,
  readOldestRetainedLine,
...
// Re-exported (module moved for the max-lines split): callers and module mocks
// address the epoch counter through pane-scroll.
export { getTerminalOutputEpoch, recordTerminalOutput } from './terminal-output-epochs'
...
export function captureScrollState(terminal: Terminal): ScrollState {
  ...
  return {
    ...
    // Why: a width-changing rewrap renumbers aterm's absolute lines; the
    // oldest-line delta lets the restore rebase captured marker lines.
    capturedOldestLine:
      !wasAtBottom && buf.type === 'normal' ? readOldestRetainedLine(terminal) : undefined
  }
}
