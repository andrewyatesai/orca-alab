  ...
  const runDeferredConnect = (): void => {
    ...
    const handleReattachResult = (
      ...
      if (connectResult?.snapshot) {
        ...
        // mis-aligning restored scrollback on any size mismatch (bug #7279). safeFit()
        // below reflows back to the pane and resizes the PTY. Mirrors
        // applyMainBufferSnapshot; the layout-only resize must not SIGWINCH the PTY.
