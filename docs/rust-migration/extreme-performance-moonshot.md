...
| `pty:data` IPC payload shape | bytes-payload vs today's string-payload, V8 structured-clone serialize+deserialize proxy [recorded 2026-07-16, `ipc-payload-serialize-bench.mjs`]: **2.3× (typical) → 22× (control-heavy) at 64KB burst chunks**, but **0.84× at 4KB ASCII** (V8's small-string fast path already wins; tiny writes are latency- not throughput-bound). String path also pays a main-side `toString` + renderer-side re-encode the bytes path skips (aterm consumes bytes). Verdict: pursue the bytes-payload `pty:data` change for burst/TUI throughput, but the REAL gate is an Electron IPC bench (this proxy omits the C++ message hop) — do not land un-Electron-benched (the same-day-bench rule) |
