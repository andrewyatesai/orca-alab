...
pub struct WinitState {
    ...
    /// The `wl_data_device_manager`, used to create a `wl_data_device` per seat
    /// for drag-and-drop. `None` if the compositor doesn't advertise it (file
    ...
    /// Monotonic id handed to each in-flight DnD read so its calloop callback can
    /// locate its `dnd_pipes` entry (calloop doesn't echo the source token back).
    pub dnd_next_id: u64,

    /// The window a file drag is currently hovering over (a `HoveredFile` was
    /// emitted for it), so `leave` knows which window to send `HoveredFileCancelled`
    /// to — the Wayland `leave` event carries no surface. `None` when no file drag
    /// is over any of our windows. See `data_device.rs`.
    pub dnd_hover_window: Option<WindowId>,
...
impl WinitState {
    pub fn new(
        ...
        for seat in seat_state.seats() {
            let mut winit_seat = WinitSeatState::new();
            // Bind a data device for each startup seat (later seats get theirs in
