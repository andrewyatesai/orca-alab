...
// nondeterministic cipher and pin the guard + payload invariants (#9381).
...
describe('persistence single-serialize save guard', () => {
  ...
  // Fork divergence from upstream: when safeStorage is unavailable the fork
  // refuses to persist secrets in plaintext (encrypt() returns '' — kept in
  // memory only). The single-serialize guard must stay deterministic through
  // that path: the on-disk slot is '' (no sentinel bytes leak), yet the guard
  // hash still reflects the in-memory plaintext so a real change is not skipped.
  it('handles empty secrets and unavailable encryption (payload has no ciphertext, guard still skips)', async () => {
    ...
    // Secrets are not written when encryption is unavailable (kept in memory);
    // and no sentinel marker ever survives into the on-disk payload.
    expect(persisted.settings.opencodeSessionCookie).toBe('')
    expect(persisted.settings.httpProxyUrl).toBe('')
    expect(raw).not.toContain('orca-secret-slot-')
