...
/// [`terminal::Terminal::cell_frame_into`]. Hosted here (not in
/// `aterm-render-api`) so the engine can emit it without a dependency cycle;
/// `aterm-render-api` re-exports it.
pub mod render;
pub mod scrollback;
pub mod search;
