  ...
  useEffect(() => {
    ...
    const manager = new PaneManager(container, {
      ...
      onPaneCreated: (pane, spawnHints) => {
        ...
        // Why: let host-handled keys bypass the pane's keyboard encoder (the aterm facade keeps xterm's
        // attachCustomKeyEventHandler name; the textarea input consults this hook BEFORE the engine encoder runs).
        // A kitty-negotiated encoder would turn app-domain chords like Cmd+C into CSI-u with preventDefault, blocking
        // Chromium's native copy; matching keyups are bypassed too so kitty release sequences don't leak after a
        // bypassed press. See xterm-bypass-policy.ts for the rule derivation.
        ...
        pane.terminal.attachCustomKeyEventHandler((e) => {
          ...
          // Why: any non-suppressed key event clears its own pending candidate
          // release before the aterm interrupt-keyup guard runs.
          clearTerminalImePendingCandidateKeyRelease(pendingTerminalImeCandidateKeyReleases, e)
          ...
          if (jisYenInput) {
            if (jisYenInput.type === 'input') {
              // Why: translated character, not a shortcut — keep it on the onData path so PTY input guards still run.
      ...
      onLinkClick: (event, url) => {
        ...
        // terminal before the click's mouseup lands, which can leave a pending
        // drag-select listener attached; clearing the selection detaches it.
        if (handled) {
      ...
      // Why: hidden panes stay mounted so PTYs survive navigation; painting them offscreen wastes GPU work and (for
      // GPU-backed drawers) context budget, so start suspended.
    ...
    // Why: the terminal renders to canvas with no a11y addon; expose the manager so E2E can read the buffer via serializeAddon.serialize().
