...
describe('shouldInspectOuterWrapperForegroundProcess', () => {
  it('flags pi (the embedded engine) for an outer-wrapper rescan', () => {
    expect(shouldInspectOuterWrapperForegroundProcess({ agent: 'pi', processName: 'pi' })).toBe(
      true
    )
  })

  it('leaves the outer omp wrapper and unrelated agents alone', () => {
    expect(shouldInspectOuterWrapperForegroundProcess({ agent: 'omp', processName: 'omp' })).toBe(
      false
    )
    expect(
      shouldInspectOuterWrapperForegroundProcess({ agent: 'codex', processName: 'codex' })
    ).toBe(false)
  })
})
