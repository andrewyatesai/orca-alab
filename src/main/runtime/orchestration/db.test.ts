...
// so stale-detection assertions stay deterministic (no wall clock). Routes
// through the Rust-store seam — the fork's OrchestrationDb is Rust-backed and
// exposes no better-sqlite3 handle to UPDATE directly.
function setDispatchTimes(
  ...
  d.setDispatchTimestamps(id, dispatchedAt, heartbeatAt)
}
