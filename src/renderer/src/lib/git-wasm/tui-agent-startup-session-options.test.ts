// Fork adaptation: the plan builders are Rust-backed via the git wasm, with the
// session-option flags layered by the TS splice (tui-agent-session-option-splice)
// — so this drives the REAL fork path (wasm core + splice), same assertions.
