...
export function shouldInspectOuterWrapperForegroundProcess(
  ...
  // Why: only Pi is currently embedded by a same-group wrapper (omp); scanning
  // omp itself would add a subprocess to every relay poll for no gain.
  return process.agent === 'pi'
}

/**
 * Collapse a foreground read onto its outermost same-title-group ancestor.
 * Why: OMP runs a shell→omp→pi tree and Orca recognizes both, but the outer omp
 * is the user-visible identity. Depth alone cannot distinguish a wrapper from a
 * sibling job, so walk ppids and only promote same-titleIdentityGroup ancestors.
 */
