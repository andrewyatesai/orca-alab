...
const SYNOPSIS: &str = "aterm-ctl [--sock PATH | --pid PID] <verb> [args...]";

/// The full `--help` text: synopsis, the socket-resolution rule, and the
/// complete VERBS reference. This mirrors the crate's top doc-comment (the
/// canonical reference) so the on-disk docs and the CLI help stay in lockstep.
const HELP: &str = "\
...
SOCKET RESOLUTION:
    ...
    are honoured as on the server), else the per-user default
    <dir>/aterm.sock where <dir> is $XDG_RUNTIME_DIR/aterm (when set) or
    ~/Library/Application Support/aterm (macOS). The default is a symlink the
    server atomically points at the newest instance's aterm-<pid>.sock, so the
    flagless flow reaches a live instance. This matches the server's
    resolution exactly.

VERBS:
    text                       print the visible screen, one row per line.
    cursor                     print OK <row> <col> <visible> <style> (style is
                               the DECSCUSR style, lowercase: blinking_block,
                               steady_block, blinking_underline,
                               steady_underline, blinking_bar, steady_bar,
                               hidden, hollow_block).
    version                    print OK version=.. commit=.. built=.. signature=..
                               (build provenance of the running aterm-gui).
    cell <r> <c>               print OK <codepoint> <fg> <bg>.
    screen                     print the FULL styled grid as one JSON frame:
                               per-cell glyph/fg/bg/attrs/underline_style/
                               overline/underline_color/emoji/wide/hyperlink,
                               plus cursor + dims + seq (lossless; no trim).
    image read [<r> [<c>]]     print inline images (OSC 1337) as base64, one line
                               each: <row> <col> <cols> <rows> <cell_row>
                               <cell_col> <format> <nbytes> <base64>. Headless +
                               cross-session safe (unlike `image`).
    search <pat>               print one \"<row> <col> <len>\" line per match.
    send <text>                write <text> to the PTY (trailing literal \\n => CR).
    key <name>                 send a named key (enter, tab, up, ...) to the PTY.
    image [path]               render the screen to a PNG; print OK <w> <h> <path>.
                               WYSIWYG: includes cursor blink phase /
    ...
    chrome                     dump the frontmost window's NATIVE macOS UI: the
                               ...
                               'menu \"<title>\": <items>' line per submenu). The
                               only verb that sees OS chrome — image/text render
                               just the terminal content view. (no native chrome
                               off macOS).
    metrics [reset]            print OK backend=<cpu|gpu> rows cols frames=<N>
                               last_/max_present_latency_ms last_/max_frame_render_ms
                               slow_frames=<N> slow_threshold_ms — live render/latency
                               counters to MEASURE responsiveness AND DETECT lag
                               without scraping logs. slow_frames>0, a big
    ...
    select <r1> <c1> <r2> <c2> select from cell (r1,c1) to (r2,c2), both
                               endpoint cells inclusive (live-screen coords;
                               negative rows reach into scrollback).
                               select clear clears the selection.
    select word <r> <c>        word-select the cell via the engine's builtin
                               smart-selection rules (URLs/paths/words; a
                               whitespace cell selects just itself) — the
                               double-click gesture.
    select line <r>            select the full line of row r (triple-click).
    select block <r1> <c1> <r2> <c2>
                               rectangular selection, the two cells as inclusive
                               corners (alt-drag).
    select extend <r> <c>      extend the existing selection so (r,c) is its new
                               inclusive endpoint (shift-click); errors with no
                               selection.
    selection                  print the selected text, one line per selected row.
    copy                       copy the selection to the system clipboard
    ...
    tab <new|N|next|prev>      drive the FRONT window's tabs: new opens a tab, N
                               ...
                               macOS tab switcher (NSSegmentedControl) in the
                               window toolbar.";
...
fn real_main() -> io::Result<ExitCode> {
    ...
    while let Some(arg) = args.next() {
        if arg == "-h" || arg == "--help" {
            ...
            // `else` branch below, which stops flag parsing).
            println!("{HELP}");
            return Ok(ExitCode::SUCCESS);
        } else if arg == "-V" || arg == "--version" {
            println!("aterm-ctl {}", env!("CARGO_PKG_VERSION"));
            return Ok(ExitCode::SUCCESS);
        } else if arg == "--sock" {
...
/// (`@<sid>` / `@.` / `@&`). Without this, a proxied `@child screen` is treated as
...
fn exchange(path: &str, request: &str, verb: &str) -> io::Result<ExitCode> {
    ...
    // `text`, `search`, `modes`, `selection`, `blocks`/`blocktext`, and `chrome`
    // stream `<n>` follow-up data lines after "OK <n>"; those lines are the
    // payload the user cares about.
    ...
    // `image read` streams `<n>` follow-up lines (one per inline image); plain
    // `image` (framebuffer rasterize) answers with only its status line.
    // `image read` detection, selector-aware: skip a leading `@<selector>` so a
...
mod tests {
    ...
    #[test]
    fn help_text_carries_the_synopsis_and_verbs() {
