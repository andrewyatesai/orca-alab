...
function createRuntimeApi(): NonNullable<Partial<PreloadApi>['runtime']> {
  return {
    ...
    // Why noop: web clients get authority via the remote multiplex stream, not local runtime IPC.
    onTerminalQueryReplyAuthorityChanged: () => noopUnsubscribe,
...
function createGitLabApi(): WebGitLabApi {
  ...
  const gitLabApi = {
    ...
    mrChecks: (args) => route<WebGitLabResult<'mrChecks'>>(GITLAB_WEB_RPC_METHODS.mrChecks, args),
    closeMR: (args) =>
