...
export function maybeRedirectAppImageCliLaunch(options: RedirectOptions = {}): RedirectResult {
  ...
  if (argv.slice(1).includes('--no-sandbox')) {
    // Why: the operator explicitly disabled Chromium's sandbox; preserve that choice when `serve` launches the Electron child (#9785).
...
export function getAppImageCliArgs(
  ...
  // Why: a desktop flag like --no-sandbox alongside a CLI command (e.g. headless
  // `serve --no-sandbox`) must still redirect to the CLI; strip the flag from the
  // forwarded args rather than falling through to GUI startup (#9785).
