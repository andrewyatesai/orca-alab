...
// The upstream suite (#7310) replayed captured writes into a headless xterm and
// inspected its buffer (baseY / scrollback lines / visible viewport) as an
// oracle. Under aterm there is no in-process VT engine in vitest (see
// terminal-replay-cursor-state.test.ts), so these tests assert the equivalent
// byte contract on the write stream itself: scrollback-preserving sequences
// only (never ED `\x1b[2J`/`\x1b[3J` or CSI S, which would erase restored
// rows), with ordering guarantees. The engine-side effect of the blanking
// sequence is pinned in terminal-restored-viewport.test.ts and the restore e2e
// specs.
function expectWritesPreserveScrollback(written: string[]): void {
  for (const chunk of written) {
    // ED 2 (viewport erase), ED 3 (scrollback erase) and CSI S (pan up,
    // discarding scrolled-out lines) would all destroy restored rows that
    // upstream proved survive in scrollback. CSI S is matched by splitting on
    // the CSI introducer instead of a control-char regex (no-control-regex).
    expect(chunk).not.toContain('\x1b[2J')
    expect(chunk).not.toContain('\x1b[3J')
    const pansUp = chunk
      .split('\x1b[')
      .slice(1)
      .some((sequence) => /^\d*S/.test(sequence))
    expect(pansUp).toBe(false)
  }
}
...
function createPane(paneId: number) {
  ...
  const terminal = {
    ...
    // Fork: the aterm facade exposes an app-notification emitter the connect
    // path subscribes to; DCS/ESC parser hooks are part of the facade parser.
...
describe('connectPanePty', () => {
  ...
  it('coalesces terminal resize forwarding while a pane resize hold is active', async () => {
    ...
    try {
      ...
      // Why: the fork's transport resize carries a claim-options third arg.
      expect(transport.resize).toHaveBeenCalledTimes(1)
      expect(transport.resize.mock.calls[0]?.[0]).toBe(120)
      expect(transport.resize.mock.calls[0]?.[1]).toBe(32)
      disposable.dispose()
    } finally {
  ...
  it('drives runtime title, tab title, and renderer policy from one title decision', async () => {
    ...
    // Display, runtime, and tab title all come from the same title decision.
  ...
  it('records hibernation activity from the DOM user-input signal, not synthetic onData replies', async () => {
    // Regression (#7283, ported to aterm): emulator auto-replies (focus in/out
    // reports, DA/DSR responses) reach the PTY through the same input seam as
    // typing. Recording them as pane input made the hibernation planner treat
    // a pane hidden after its agent finished as "input after done" and never
    // hibernate it. The fork classifies real input at the pane DOM — gestures
    // auto-replies never produce.
    const { connectPanePty } = await import('./pty-connection')
    const transport = createMockTransport('pty-pane-2')
    transportFactoryQueue.push(transport)
    const manager = createManager(1)
    ...
    // A focus-out report forwarded to the PTY must not count as activity: with
    // the DOM signal live, accepted-send fallback recording is disabled.
    ...
    // A real keystroke gesture on the pane DOM records activity.
    terminalTarget.dispatch(keyEvent({ key: 'a' }))
    expect(mockStoreState.recordTerminalInput).toHaveBeenCalledTimes(1)

    binding.dispose()
    // Dispose detaches the signal's gesture listeners with everything else.
    expect(terminalTarget.handlers.size).toBe(0)
  })

  it('falls back to onData hibernation recording when the DOM user-input signal is unavailable', async () => {
    // A pane whose target cannot host DOM listeners (the fixture's plain-object
    // element) must degrade to the historical accepted-send recording — never
  ...
  it('removes the terminal key listener on dispose before a remount adds another', async () => {
    ...
    // Two keydown listeners per connect: the attention listener and the DOM
    // user-input signal's gesture classifier.
    const keydownListenersPerConnect = terminalTarget.handlers.size
    expect(keydownListenersPerConnect).toBe(2)
    const addCallsPerConnect = (terminalTarget.target.addEventListener as ReturnType<typeof vi.fn>)
      .mock.calls.length

    firstConnection.dispose()
    expect(terminalTarget.handlers.size).toBe(0)
    // Every listener the connect attached (keydown + the signal's paste/
    // mousedown/wheel) is detached on dispose — nothing survives to stack.
    expect(terminalTarget.target.removeEventListener).toHaveBeenCalledTimes(addCallsPerConnect)

    connectPanePty(pane as never, createManager(1) as never, createDeps() as never)
    expect(terminalTarget.handlers.size).toBe(keydownListenersPerConnect)
    expect(terminalTarget.target.addEventListener).toHaveBeenCalledTimes(addCallsPerConnect * 2)
  })
  ...
  it('clears the stale restored frame when the daemon respawns a fresh session on reattach', async () => {
    const { connectPanePty } = await import('./pty-connection')
    // Why 'tab-pty': the reattach payload-currency guard compares the payload's
    // ptyId against transport.getPtyId(); the connect override below does not
    // update the mock's internal id, so seed it (same as the sibling tests).
    const transport = createMockTransport('tab-pty')
    transport.connect.mockImplementation(async ({ sessionId }: { sessionId?: string }) => {
  ...
  it('retries a fresh-spawn native follow reset when renderer dimensions return', async () => {
    ...
    pane.terminal.buffer.active.baseY = 100
    // Fork: the aterm facade has no onRender; the fresh-spawn follow reset
    // retries on onResize (the earliest facade signal that the pane became
    // drivable at real dimensions). Capture every resize subscription so the
    // retry listener can be driven and its cleanup observed.
    const resizeSubscriptions: {
      listener: (size: { cols: number; rows: number }) => void
      dispose: ReturnType<typeof vi.fn>
    }[] = []
    pane.terminal.onResize = vi.fn((listener: (size: { cols: number; rows: number }) => void) => {
      const dispose = vi.fn()
      resizeSubscriptions.push({ listener, dispose })
      return { dispose }
    }) as never
    ...
    expect(resizeSubscriptions.length).toBeGreaterThan(0)

    for (const subscription of resizeSubscriptions.slice()) {
      subscription.listener({ cols: 120, rows: 40 })
    }
    expect(pane.terminal.buffer.active.viewportY).toBe(100)
    expect(pane.terminal.scrollToBottom).toHaveBeenCalledTimes(2)
    expect(getTerminalScrollIntentKind(pane.terminal)).toBe('followOutput')
    // The successful retry must dispose exactly its own retry subscription.
    expect(
      resizeSubscriptions.filter((subscription) => subscription.dispose.mock.calls.length === 1)
    ).toHaveLength(1)
  })
  ...
  it('blanks restored scrollback before fresh shell output', async () => {
    ...
    // Upstream rendered [restored rows, blankViewport, 'PS >'] through a
    // headless xterm and proved the restored rows moved into scrollback
    // (baseY > 0) while the viewport showed only the fresh prompt. Without an
    // in-process engine, the same guarantee is the byte contract: nothing the
    // pane wrote can erase the restored rows, and the blanking sequence scrolls
    // exactly one CRLF per viewport row before homing the cursor for 'PS >'.
    expectWritesPreserveScrollback(written)
  ...
  it('cold-restores at the recovered grid and clears only the dirty viewport before blanking', async () => {
    ...
    // Upstream replayed `written` + a fresh prompt through a headless xterm and
    // proved the cold-restored rows survived in scrollback (baseY > 0) with a
    // blank viewport for the prompt. The fork-equivalent guarantee on the write
  ...
  // #7596: a plain terminal's cold restore offers the recovered last command.
  it('shows the restored banner with lastCommand for a plain-terminal cold restore', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const transport = createMockTransport('fresh-pty')
    transport.connect.mockImplementation(async ({ sessionId }: { sessionId?: string }) => {
      if (sessionId) {
        return {
          id: 'fresh-pty',
          coldRestore: { scrollback: 'cold-payload', cwd: '/tmp/wt-1', lastCommand: 'npm run dev' }
        }
      }
      return 'fresh-pty'
    })
    ...
    expect(pane.terminal.write).toHaveBeenCalledWith('cold-payload', expect.any(Function))
    expect(deps.onShowSessionRestoredBanner).toHaveBeenCalledTimes(1)
    expect(deps.onShowSessionRestoredBanner).toHaveBeenCalledWith(1, { lastCommand: 'npm run dev' })
    // The affordance types on demand — never auto-runs anything at restore.
    expect(transport.sendInput).not.toHaveBeenCalled()
  })

  it('offers no lastCommand banner for multiline or missing commands', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const transport = createMockTransport('fresh-pty')
    transport.connect.mockImplementation(async ({ sessionId }: { sessionId?: string }) => {
      if (sessionId) {
        return {
          id: 'fresh-pty',
          // Multiline commands are ineligible: the banner ellipsis is not a shell.
          coldRestore: { scrollback: 'cold-payload', cwd: '/tmp/wt-1', lastCommand: 'a\nb' }
        }
      }
      return 'fresh-pty'
    })
  ...
  // #7596 gating: agent panes relaunch their own CLIs — never the affordance.
  it('agent-resume cold restore never carries the lastCommand affordance', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const transport = createMockTransport('fresh-pty')
    transport.connect.mockImplementation(async ({ sessionId }: { sessionId?: string }) => {
      if (sessionId) {
    ...
    expect(deps.onShowSessionRestoredBanner).toHaveBeenCalledTimes(1)
    // Agent resume shows the plain banner (no lastCommand payload argument).
    expect(deps.onShowSessionRestoredBanner).toHaveBeenCalledWith(1)
  })
  ...
  describe('hidden-delivery gate', () => {
    ...
    describe('foreground flood restore feedback loop (rc.7.perf)', () => {
      ...
      it('abandons the restore on queue overflow, writes the stream through, and repaints once', async () => {
        ...
        try {
          vi.useFakeTimers()
          // Fork: carry more scrollback rows than the live buffer (baseY=100)
          // or the richer-live-scrollback guard skips the clear+replay.
  ...
  it('does not apply stale background Codex query chunks after hidden snapshot restore', async () => {
    ...
    // Upstream rendered `writes` and the full frame stream through a headless
    // xterm and asserted buffer equality: skipping the snapshot-covered stale
    // frame must not corrupt the final render. Without an in-process engine,
    // assert the same outcome on the stream: the stale frame's content and its
    // DSR query never reach the terminal (a forwarded `\x1b[6n` would provoke
    // a phantom cursor-position reply), and the last spinner frame the pane
    // rendered is the current one the snapshot and in-order chunks agree on.
    for (const chunk of writes) {
      expect(chunk).not.toContain('Working stale')
      expect(chunk).not.toContain('\x1b[6n')
    }
    const spinnerWrites = writes.filter((chunk) => chunk.includes('Working'))
    expect(spinnerWrites.length).toBeGreaterThan(0)
    expect(spinnerWrites.at(-1)).toContain('Working current')
    binding.dispose()
  })
  ...
  it('writes ordinary hidden output live instead of proactively restoring a snapshot', async () => {
    ...
    expect(getMainBufferSnapshot).not.toHaveBeenCalled()
    expect(pane.terminal.write).toHaveBeenCalledWith(hidden, expect.any(Function))
  ...
  it('cancels a delayed snapshot scroll restore when the pane binding is disposed', async () => {
    ...
    const hidden = 'x'.repeat(2 * 1024 * 1024 + 1)
    const live = 'visible-after\r\n'
    // Fork: the snapshot must carry MORE scrollback rows than the live buffer
    // (baseY=100, rows=40) or the richer-live-scrollback guard skips the
    // destructive clear+replay entirely (see applyMainBufferSnapshot).
    const snapshotData = 'snapshot-state\r\n'.repeat(150)
    getMainBufferSnapshot.mockResolvedValue({
      data: snapshotData,
      cols: 100,
    ...
    expect(writes).toContain(snapshotData)
  ...
  it('forces a viewport refresh when the foreground CSI introducer is split across PTY chunks', async () => {
    ...
    // aterm shapes complex scripts natively; the xterm-era per-pane renderer
    // switch (markPaneHasComplexScriptOutput) no longer exists.
    expect(refresh).toHaveBeenCalledWith(0, 39, true)
    expect(scheduleTerminalWebglAtlasRecovery).toHaveBeenCalledTimes(1)
  })
  ...
  // Upstream (#7447) also asserts per-pane GPU rendering is re-enabled here via
  // manager.setPaneGpuRendering(1, true). aterm has no per-pane GPU toggle (see
  // pty-connection.ts), so we keep only the title-labeling assertions — the OMP-
  // not-Gemini mislabeling fix — which is the part that applies to this fork.
  it('keeps the OMP title for OMP terminals whose cwd mentions Gemini', async () => {
    const { connectPanePty } = await import('./pty-connection')
  ...
  it('queues the idle cursor reset behind hidden agent output', async () => {
    ...
    vi.advanceTimersByTime(50)
    expect(pane.terminal.write).toHaveBeenCalledWith(
      `\x1b[6 q${RESET_TERMINAL_CURSOR_STYLE}`,
      expect.any(Function)
    )
  })
