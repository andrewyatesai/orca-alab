...
async function xtermMBs() {
  ...
  let best = 0
  for (let i = 0; i < 5; i++) {
    best = Math.max(best, await onePass())
  }
  return best
}
...
for (let f = 1; f <= frames; f++) {
  ...
  if (isTTY) {
    process.stdout.write(`\x1b[s${line}\x1b[u`)
    await new Promise((r) => setTimeout(r, (dur * 1000) / frames))
  } else if (f === frames) {
