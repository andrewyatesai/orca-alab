...
// Why cap coalescing: unbounded same-session `+=` grows a held entry into a multi-MB V8 ConsString whose every
// drain-cycle slice() re-flattens the remainder — O(n²) stalls. Capping makes the queue a head-indexed chunk list
// (each entry flattened once); no framing is lost — flush slices at BULK_WRITE_SLICE_CHARS regardless.
...
export class DaemonStreamDataBatcher {
  ...
  enqueue(
    ...
    // Why the empty seed: once the coalesced tail reaches the cap, start a fresh chunk —
    // appendDaemonStreamData coalesces onto the seed (same conditions as this guard), so
    // no entry ever grows past MAX_COALESCED_ENTRY_CHARS + one enqueue.
    if (
