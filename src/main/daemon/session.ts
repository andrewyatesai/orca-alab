...
export class Session {
  ...
  constructor(opts: SessionOptions) {
    ...
    this.emulator = createHeadlessEmulator({
      ...
      // No onQueryReply sink: the daemon-side emulator stays write-only and
      // must never answer terminal query sequences. Query authority lives in
      // main's per-PTY runtime emulators, which reply only for chunks the
      // hidden-delivery gate dropped (terminal-query-authority.md); a daemon
      // reply would race that path. See the header comment in HeadlessEmulator.
    })
    ...
    this.startupIngress = new PtyStartupIngress({
      ...
      // Why through the guard: a reply must not reorder around a held conin tail.
      write: (data) => this.writeToSubprocess(data),
