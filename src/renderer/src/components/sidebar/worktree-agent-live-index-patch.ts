...
/**
 * Returns a refreshed cache when a within-state ping can be served by patching
 * the owning worktree's bucket, or null to signal the caller must full-rebuild.
 * Keeps the O(all live agents) fallback and the cache write in the selector.
 */
export function maybePatchLiveEntriesCache(
  cache: LiveEntriesByWorktreeCache | null,
  tabsByWorktree: AppState['tabsByWorktree'],
  agentStatusByPaneKey: AppState['agentStatusByPaneKey'],
  tabIdToWorktreeId: Map<string, string>
): LiveEntriesByWorktreeCache | null {
  if (cache?.tabsByWorktree !== tabsByWorktree) {
    return null
  }
  const patched = patchLiveEntriesByWorktree(cache, agentStatusByPaneKey, tabIdToWorktreeId)
  if (!patched) {
    return null
  }
  return { tabsByWorktree, agentStatusByPaneKey, entriesByWorktree: patched }
}
