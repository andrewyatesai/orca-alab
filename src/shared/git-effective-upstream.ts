...
export async function resolveEffectiveGitUpstream(
  ...
  return resolveEffectiveGitUpstreamForBranch(runGit, isNoUpstream, await getCurrentBranchName(runGit))
}
...
export async function getEffectiveGitUpstreamStatus(
  ...
  const upstream = await resolveEffectiveGitUpstreamForBranch(runGit, isNoUpstream, currentBranchName)
  if (!upstream) {
