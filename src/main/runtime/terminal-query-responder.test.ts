...
describe('reply parity for hidden-dropped chunks', () => {
  // Expected replies pinned from the design doc. The runtime emulator is the
  // aterm-backed HeadlessEmulator; its query responder answers from engine
  // state (cursor for CPR, modes) + the renderer's pushed view attributes,
  // byte-for-byte with what a visible aterm pane (xterm.js identity) replies.
  it.each([
...
describe('HeadlessEmulator forwarding window', () => {
  ...
  it('scopes the forwarding window to the flagged chunk parse', async () => {
    const onQueryReply = vi.fn()
    const emulator = new HeadlessEmulator({ cols: 80, rows: 24, onQueryReply })
    try {
      // An unflagged seed carrying a query, then a flagged live chunk. aterm
      // parses each write synchronously, so the forwarding window is inherently
      // bracketed to a single chunk: the seed's DA1 answers no one while the
      // flagged DSR does.
