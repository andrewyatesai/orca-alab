...
describe('buildRows with pinned worktrees', () => {
  ...
  it('labels same-repo worktrees by runtime host in the All view (#10061)', () => {
    const firstRepo: Repo = {
      ...repo,
      id: 'repo-runtime-a',
      executionHostId: 'runtime:env-a'
    ...
    const firstWorktree: Worktree = { ...worktree, id: 'wt-runtime-a', repoId: firstRepo.id }
    const secondWorktree: Worktree = { ...worktree, id: 'wt-runtime-b', repoId: secondRepo.id }
    const rows = buildRows(
      'none',
      [firstWorktree, secondWorktree],
      new Map([
  ...
  it('omits per-worktree host labels in the All view when every worktree shares one host', () => {
    const secondLocalWorktree: Worktree = { ...worktree, id: 'wt-local-2', displayName: 'local-2' }
    const rows = buildRows(
      'none',
