...
//!
//! Owned here so production (napi + wasm) and the parity harness share ONE
//! registry. `orchestration-store` is the sole exception: it needs rusqlite/
//! orca-runtime, so it stays local to orca-parity, which layers it over this
//! aggregate dispatch.
