...
// forever, so after this many restarts the find settles with ONE blocking one-shot
// search — the pre-P1.1 behavior, now the bounded worst case instead of the norm.
...
export function createSlicedFindRunner(
  ...
  const start = (
    ...
    const slice = (): void => {
      ...
      // restart forever, so settle with ONE blocking one-shot search instead.
      if (cursor !== undefined && step.rowsFed <= rowsSeen) {
        restarts++
        if (restarts >= SEARCH_FIND_MAX_RESTARTS) {
          handle.searchBudgetedCancel?.()
          const t1 = performance.now()
          const found = decodeMatches(handle.search(q, cs, regex))
