...
// No arabic-shaping joiner import: aterm-bidi shapes RTL natively in the engine.
...
// Why: a title completion suppressed by fresh hook status is normally a mid-
// turn flap the hook's own events resolve within seconds; when the agent died
// without its final hook (#7202) no such event ever comes, so after this quiet
// window an untouched working row lets the held title complete and repair it.
...
export function connectPanePty(
  ...
  // `reset` (fork): the agent-done subscription below passes an explicit
  // kitty-inclusive reset for aterm panes; callers with no arg use the pane's
  // idle reset default.
  ...
  let suppressedTitleCompletionRepairTimer: ReturnType<typeof setTimeout> | null = null
  const clearSuppressedTitleCompletionRepair = (): void => {
    if (suppressedTitleCompletionRepairTimer !== null) {
      clearTimeout(suppressedTitleCompletionRepairTimer)
      suppressedTitleCompletionRepairTimer = null
    }
  }
  // Why: fresh hook status outranks title flaps (#7818), but a Codex fatal
  // error can end the turn without the final hook (#7202). Hold the suppressed
  // title; if the working row sees zero hook activity for the settle window,
  // the completion was genuine — replay it so the repair path can run.
  const armSuppressedTitleCompletionRepair = (
    suppressedRow: AgentStatusEntry,
    replayTitleCompletion: () => void
  ): void => {
    clearSuppressedTitleCompletionRepair()
    if (suppressedRow.state !== 'working') {
      return
    }
    const baselineUpdatedAt = suppressedRow.updatedAt
    suppressedTitleCompletionRepairTimer = setTimeout(() => {
      suppressedTitleCompletionRepairTimer = null
      const currentEntry = useAppStore.getState().agentStatusByPaneKey[cacheKey]
      // Why: any hook update during the settle window proves the turn is
      // alive; only the byte-identical stuck row may complete via title.
      if (
        !currentEntry ||
        currentEntry.state !== 'working' ||
        currentEntry.updatedAt !== baselineUpdatedAt
      ) {
        return
      }
      replayTitleCompletion()
    }, SUPPRESSED_TITLE_COMPLETION_REPAIR_SETTLE_MS)
  }
  ...
  const handleCommandFinished = (bestEffortExitCode: number | null): void => {
    ...
    const longCommandDecision = longCommandNotificationTracker.onCommandFinished(bestEffortExitCode)
  ...
  const registerSideEffectFactConsumerForPty = (ptyId: string): void => {
    ...
    unregisterSideEffectFactConsumer = registerTerminalSideEffectFactConsumer({
      ...
      callbacks: {
        ...
        // Why: main's tracker scrapes the Codex fatal stream-error line under
        // side-effect authority; the pane policy repairs the stuck row (#7202).
        onCodexStreamError: completeCodexOutputStreamErrorStatus,
  ...
  // Why: the repair rewrites the row to 'done' before the completion
  // notification captures its schedule-time baseline; the newer-turn veto must
  // compare against the pre-repair turn or a same-turn hook refresh would read
  // as a new turn and swallow the notification. Consumed within the same task.
  let agentStatusBaselineBeforeRepair: AgentStatusEntry | null = null
  ...
  const completeWorkingStatusFromCompletionSignal = (
    ...
    agentStatusBaselineBeforeRepair = currentEntry
    queueMicrotask(() => {
      agentStatusBaselineBeforeRepair = null
    })
  ...
  const agentCompletionCoordinator = createAgentCompletionCoordinator({
    ...
    dispatchCompletion: (title, meta) => {
      ...
      if (meta?.terminalIdleConfirmed === true) {
        ...
        // The pre-repair baseline keeps the fresh-working veto intact after the
        // #7202 repair rewrites the row to done in this same task.
        const currentAgentStatus =
          agentStatusBaselineBeforeRepair ?? useAppStore.getState().agentStatusByPaneKey[cacheKey]
  ...
  const onTitleChange = (
    ...
    // NOTE: upstream toggles per-pane GPU/WebGL rendering here via
    // decision.rendererPolicy; aterm has no per-pane GPU toggle and renders
    // Gemini fine, so that xterm-only workaround stays removed.
    ...
    if (!meta?.staleWorkingTitleClear && syncAgentTaskCompleteTrackingEnabled()) {
      ...
      } else if (activeHookStatus) {
        armSuppressedTitleCompletionRepair(activeHookStatus, () => {
  ...
  const bindActivePanePty = (
    ...
    if (activePanePtyBinding !== ptyId) {
      // Why: a rebound PTY starts a fresh byte stream; stale Codex fatal-line
      // carry from the previous PTY must not complete the new session's turn.
  ...
  const scheduleAgentTaskCompleteNotification = (
    ...
    // Why: prefer the pre-repair row (same synchronous task) so the newer-turn
    // veto compares turn identity against the turn that actually completed.
    const agentStatusAtSchedule =
      agentStatusBaselineBeforeRepair ?? useAppStore.getState().agentStatusByPaneKey[cacheKey]
  ...
  // their idle reset already carries the kitty clear (see above). On native
  // Windows ConPTY this ALSO drives Codex focus-report suppression (#8842):
  // arm it on a completed turn, disarm on any non-done transition.
  if (isNativeWindowsConpty) {
  ...
  unsubscribeAgentDoneTerminalModeReset = useAppStore.subscribe((nextState) => {
    ...
    if (nextAgentStatusState === 'done') {
      ...
      } else if (lastAgentStatusState !== 'done' && pane.atermController) {
        queueAgentIdleTerminalModeReset(
          `${idleAgentTerminalModeReset}${RESET_KITTY_KEYBOARD_PROTOCOL}`
        )
      }
    } else if (isNativeWindowsConpty && nextAgentStatusState) {
      suppressNativeWindowsIdleCodexFocusReports = false
    }
  ...
  // Why (byte-parser mode only): with main authority the Codex fatal-line
  // scrape runs in main's per-PTY tracker and arrives as codex-stream-error
  // facts; running the byte detector too would double-drive the repair policy.
  const codexErrorOutputStatusDetector = mainSideEffectAuthority
    ? null
  ...
  // Why: the PTY input pipeline mixes real user input with the emulator's
  // auto-replies (focus reports, DA/DSR/CPR drains). Recording those replies as
  // activity makes the hibernation planner treat a pane hidden after its agent
  // finished as "input after done" forever. The DOM user-input signal fires only
  // for real input gestures, so hibernation activity records from it; accepted-
  // send recording remains solely as the fallback when the signal is unavailable.
  ...
  const respondToTerminalPixelSizeQueries = createTerminalPixelSizeQueryResponder(
    pane.terminal,
    // Why sendDesktopQueryReplyImmediate: pixel-size (14t/16t) replies must beat
    // the querying program's raw-mode read window; the transport's input debounce
    // would corrupt them (#7329), so send immediately like the other capability
    // replies.
  ...
  const runDeferredConnect = (): void => {
    ...
    // Also gate on visibility: background/hidden tabs (orchestration workers, CLI
    ...
    if ((cols === 0 || rows === 0) && pane.atermController && deps.isVisibleRef.current) {
      ...
      const unregisterSerializer = registerPtySerializer(
        ...
        async (opts) => {
          try {
            ...
            // output + off-screen history (its sync serialize can't reach them). The
            // aterm engine places every row with absolute CUP + erase-line and restores
            // the cursor absolutely, so it supersedes upstream's serializeWithAbsoluteCursor
            // wrap-pending fix on the xterm SerializeAddon path.
            const data =
    ...
    const resetFreshSpawnFollowOutput = (): void => {
      ...
      if (!nativeFollowResetComplete) {
        // Why: retry once geometry lands. The aterm facade has no onRender
        // (a pre-attach scrollToBottom is a silent no-op on a fresh engine
        // already at bottom, and never throws); onResize is the earliest
        // facade signal that the pane became drivable at real dimensions.
        freshSpawnFollowResetDisposables = [pane.terminal.onResize(tryResetNativeFollow)]
      }
    }
    ...
    const drainReplayDataQueue = async (
      ...
      while (pendingReplayData !== null) {
        ...
        // Why: remote-runtime snapshots can arrive after the renderer attached to
        // an empty buffer; repaint after replay parses so the pane presents the
        // now-populated state (aterm: a fresh scheduleDraw, no atlas to rebuild).
    ...
    function beforeTerminalOutputWrite(_data: string): void {
      // aterm-bidi shapes RTL/Arabic natively in the engine — no renderer-side
      // character-joiner registration needed (unlike the removed xterm path).
            ...
            afterRestore: async () => {
              ...
              const fit = safeFitAndThen(
                ...
                () => {
                  ...
                  // Why (fork): a restore-time SIGWINCH makes alternate-screen TUIs
                  // (Codex, vim) rebuild their full-screen layout to the TOP,
                  // discarding the snapshot we just replayed. Suppress the
                  // resize+SIGWINCH for any alt-screen restore; a genuine window
                  // resize is reconciled by the pane's own resize observer when the
                  // hidden tab becomes visible, so the child still learns its size.
    ...
    const dataCallback = (
      ...
      if (codexErrorOutputStatusDetector && !codexStreamErrorOutputFastPathConsumed) {
    ...
    const handleReattachResult = async (
      ...
      const applyReattachPayload = async (): Promise<void> => {
        ...
        if (connectResult?.snapshot) {
          ...
          // Why (fork): the daemon lost the session we tried to reattach to and
          // spawned a fresh shell with no history to replay.
          // restoreScrollbackBuffers() already painted the previous session's
          // last frame at mount; clear it so the fresh shell starts clean
          // instead of under stale output. No live TUI owns the mode state
          // here, so also reset the crashed-TUI modes.
          writeReplayData('\x1b[2J\x1b[3J\x1b[H')
          writeReplayData(POST_REPLAY_MODE_RESET)
        }
      ...
      // Why (fork): POSIX only delivers SIGWINCH on a real dims change, so the
      // reattach path signals explicitly to make restored TUIs repaint — on the
      // NORMAL screen only. Stand down when the pane is on the alt screen (the
      // TUI owns its full frame; a forced repaint makes some TUIs rebuild their
      ...
      // alt-screen flag once it replays — signaling first races the replay and
      // the TUI's top-rebuild frame lands last, clobbering it).
      const maybeSignalReattachSigwinch = (reattachPtyId: string): void => {
        ...
        if (!getFitOverrideForPty(reattachPtyId)) {
          const fit = safeFitAndThen(
            ...
            () => {
              ...
              maybeSignalReattachSigwinch(reattachPtyId)
            },
  ...
  const reconcileIfSessionMissing = (
    ...
    void livenessPromise
      .then((isLive) => {
        ...
        // Why: same as reconcileIfSessionDead — a PTY missing from main is an
        // unexpected death, so a sole pane must get the recovery overlay.
        onExit(currentPtyId, 1)
      })
      .catch(() => {})
  }
