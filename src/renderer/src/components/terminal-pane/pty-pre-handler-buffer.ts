...
// Why: distinguishes a fully-captured dead parked session (final output already
// buffered here) from a bare exit signal that raced a reattach. The former has
// nothing to gain from respawning; the latter must still reattach so the daemon
// snapshot survives the race (see connect()'s deferred-drain).
export function hasPreHandlerPtyData(ptyId: string): boolean {
  return preHandlerPtyData.has(ptyId)
}
