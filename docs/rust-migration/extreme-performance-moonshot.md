...
**Consequence for the factory:** the reservoir the section below theorizes about is now a PROVEN equivalence corpus (W1 ∀-safety ∧ W2 0-divergence), and the honest lever was **matching/allowlist correctness + harness fidelity + reformulation**, not (primarily) nonlinear/loop-invariant theory (those remain real T-D/T-E investments for the harder shipped ports, but were NOT the bottleneck for the corpus). Raw material for F4/P-ports is ready; the open work is **promotion** (proven kernel → shipped napi/wasm shadow), not proving. Historical diagnosis retained below for provenance.

**✅ CORPUS GROWN ON FRESH REAL-ORC CODE + F2 FIRST RUNG LANDED — 2026-07-18 (ratchet floor `tools/terminal-bench/autoformalize-ratchet.json` = 258):** after the 100% (243), the factory manual loop (`grow-corpus` workflow) formalized **+15 FRESH orc functions the curated corpus had never seen** (clipboard byte-length, WSL/URI path transforms, slash-command/pi-legacy title predicates, session-id injection guard, VM-recipe secret redaction, braille-glyph strip, host-tab-id validation, the **git credential scrubber**, cert-error codes, …) → **258/264 TRUSTED, 100% of the 258 non-control, 6 controls refused, 0 residual** — proving the pipeline generalizes, not just the curated set. It also produced **principled SKIPs** that *evidenced* a real boundary: the generic fuzzer's fixed STR/number pool can't reach domain-specific trigger shapes (a `scheme://user:pass@` credential, a magic code in a narrow band), so W2 there degenerates to a hollow `identity==identity` — the factory correctly refuses a vacuous TRUSTED. That motivated the **F2 first rung** (`verify.mjs` + a per-kernel `<base>.seed.jsonl` the differential AUGMENTS onto the generic corpus, still diffed TS-oracle-vs-port): it recovered `stripCredentialsFromMessage` (**a security function now with a machine-checked panic-freedom + faithfulness proof** — W1 12/12, W2 0/61 over credential-URL / ssh-untouched / BOM-inside-credential seeds) and `isChromiumCertificateErrorCode`. Remaining F2 work: trace-DERIVED (not just hand-authored) seed corpora, and the one deferred near-miss `hasCompatibleAgentTitleIdentity` (W2-faithful 0/76 but 1 W1 runtime-checked obligation on its heavy title-regex cascade).
