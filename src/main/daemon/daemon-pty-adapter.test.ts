...
describe('DaemonPtyAdapter (IPtyProvider)', () => {
  ...
  describe('history integration', () => {
    ...
    // Why keep the fork's deletion test (not upstream's endedAt-stamp variant):
    // the fork deletes at-rest history on a clean live-stream exit so scrollback
    // secrets never outlive the session; upstream's stamp-and-keep model is
    // superseded here.
