...
/// Kitty `REPORT_ALTERNATE_KEYS` 3rd field carried by `base_layout`. `None` when
...
/// Non-macOS fallback for [`build_key_input`]: `key_without_modifiers` is a
/// platform extension trait, so off macOS we map the plain logical key. (aterm-gui
