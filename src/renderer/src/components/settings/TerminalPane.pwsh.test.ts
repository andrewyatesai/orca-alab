...
describe('TerminalPane PowerShell version setting', () => {
  ...
  it('shows a persisted WSL default without PowerShell version or distro sub-settings', () => {
    const element = TerminalPane({
      settings: {
        terminalScrollbackRows: 5_000,
    ...
    expect(text).toContain('PowerShell')
    expect(text).toContain('Command Prompt')
    // Why (#9779): the real WSL default must be shown as its own (disabled) segment, not mislabeled as PowerShell.
