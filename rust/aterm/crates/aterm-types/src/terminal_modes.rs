...
/// Terminal mode flags.
//
// `PartialEq, Eq` enable by-value comparison in `TerminalCheckpoint` (B.3.2):
// every field is itself `PartialEq, Eq` (bools + Copy enums), so the derive is
// exact and adds no new invariants.
