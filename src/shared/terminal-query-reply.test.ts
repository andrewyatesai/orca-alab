...
// The exact DCS-framed XTVERSION string the daemon query responder answers
// CSI > q with (aterm mirrors the xterm.js identity). Pinned here so the
// classifier test needs no live terminal engine.
...
describe('isTerminalQueryReply', () => {
  ...
  it('classifies the fully framed XTVERSION reply the query responder emits', () => {
    // The daemon query responder answers CSI > q with this exact DCS-framed
    // XTVERSION string (aterm mirrors the xterm.js identity); the classifier
    // must route it on the immediate reply path like the CPR/OSC replies.
    expect(XTVERSION_REPLY.startsWith('\x1bP>|xterm.js(')).toBe(true)
    expect(XTVERSION_REPLY.endsWith(')\x1b\\')).toBe(true)
    expect(isTerminalQueryReply(XTVERSION_REPLY)).toBe(true)
  })
