...
fn read_integer_ptr(
    ...
    loop {
        ...
        *input_ptr = unsafe { input_ptr.add(1) };
        // Saturating: a long `0xFF` run must not overflow (the oversized length is
        // then rejected by the downstream bounds checks). Matches the safe decoder.
...
pub(crate) fn decompress_internal<const USE_DICT: bool, S: Sink>(
    ...
    loop {
            ...
            && output_ptr < safe_output_ptr
        {
            ...
            // NOTE ON SCOPE: this whole module is the UNSAFE decoder, compiled
            // only under `#[cfg(not(feature = "safe-decode"))]`. The crate's
            // default features include `safe-decode`, and aterm-scrollback (the
            // only consumer) does not disable defaults — so in the shipped aterm
            // build the bounds-checked `decompress_safe` path runs instead and
            // THIS code is not compiled. These guards harden the raw-pointer
            // decoder for builds that opt out of `safe-decode`; they are not the
            // production OOB defense (the safe sink's slice indexing is).
            //
            // Soundness guard (hot path): the fixed 16-byte literal copy and
            // the fixed 18-byte match copy below both write/read more bytes
            // than `literal_length`/`match_length` to stay branch-light. The
            // heuristic `safe_output_ptr` / `input_ptr_safe` guards keep this
            // in bounds for *well-formed* input, but the lengths are derived
            // from attacker-influenced bytes. Verify the wide copies fit in
            // both buffers before issuing them; bail to the decode error
            // otherwise. (Given the loop entry condition this is belt-and-
            // suspenders — see the adjacent debug_assert — but it converts any
            // future entry-guard regression into a clean error, not an OOB.)
            let out_remaining = unsafe { output_end.offset_from(output_ptr) as usize };
            let in_remaining = unsafe { input_ptr_end.offset_from(input_ptr) as usize };
            if out_remaining < 16 || in_remaining < 16 {
            ...
            if offset >= match_length {
                // Soundness guard: the optimized copy writes a fixed 18 bytes
                // at `output_ptr` regardless of `match_length`. The read at
                // `start_ptr = output_ptr - offset` stays within the already
                // written output, so the binding constraint is that the 18
                // destination bytes fit before `output_end`. Bail to the
                // decode error rather than writing past the buffer when the
                // (attacker-influenced) lengths/offset would overrun.
                if unsafe { output_end.offset_from(output_ptr) as usize } < 18 {
                    return Err(DecompressError::OutputTooSmall {
                        expected: unsafe { output_ptr.offset_from(output_base) as usize }
                            + match_length,
                        actual: output.capacity(),
...
mod test {
    ...
    /// Regression: a crafted block whose literal length exceeds a too-small
    /// output buffer must return a decode `Err`, never OOB-write or panic.
    /// `0xE0` requests 14 literals into a 4-byte output. The bounds guards
    /// (hot-path soundness check and slow-path validation) must reject it.
    #[test]
    fn hot_loop_literal_overruns_small_output_errors() {
        // token 0xE0 => literal_length 14, match nibble 0. Followed by 14
        // literal bytes, but the output buffer is only 4 bytes.
        let mut input = alloc::vec![0xE0u8];
        input.extend_from_slice(&[b'x'; 14]);
        input.extend_from_slice(&[0u8, 0u8]); // dummy offset bytes
        let mut output = [0u8; 4];
        decompress_into(&input, &mut output)
            .expect_err("oversized literal must error, not OOB-write");
    }

    /// Regression: a block whose match length overruns a near-full output
    /// must yield a decode `Err` rather than reading/writing out of bounds.
    /// This exercises the match-copy path with a tiny output buffer.
    #[test]
    fn hot_loop_match_overruns_small_output_errors() {
        // token 0x1E => literal_length 1, match length 14+MINMATCH. One
        // literal byte then a back-reference whose match copy would overrun a
        // 2-byte output. Must error, never panic / OOB.
        let input = alloc::vec![0x1Eu8, b'a', 1, 0];
        let mut output = [0u8; 2];
        decompress_into(&input, &mut output)
            .expect_err("oversized match must error, not OOB-write");
    }

    /// Regression: a crafted block sized to enter the hot path (large enough
    /// output that `safe_output_ptr` advances past `output_ptr`) but whose
    /// literal/offset bytes are adversarial must still stay within bounds —
    /// the per-copy soundness guards ensure no panic / OOB.
    #[test]
    fn hot_loop_adversarial_block_stays_in_bounds() {
        // 64-byte output makes the hot path reachable; feed a malformed
        // stream and require a clean result (Ok or Err), never UB.
        let input = alloc::vec![
            0x5Au8, b'a', b'b', b'c', b'd', b'e', 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
        ];
        let mut output = [0u8; 64];
        // Must not panic or read/write out of bounds regardless of outcome.
