...
// The worker-backed term must correlate posted finds with the STATE echo
// (searchGeneration): until the worker echoes the newest posted generation, the
// snapshot count is the PREVIOUS query's, so searchStateSnapshot flags `pending`
...
function makeBackedTerm(): {
  ...
  finds: AtermWorkerSearchFind[]
} {
  const finds: AtermWorkerSearchFind[] = []
  const backed = createWorkerBackedTerm({
    post: (cmd: AtermWorkerPaneCommand) => {
      if (cmd.type === 'searchFind') {
        finds.push(cmd)
      }
    },
...
describe('worker-backed term search pending state', () => {
  it('posts a monotonic generation per find and pends until the STATE echo catches up', () => {
    ...
    expect(finds.map((f) => f.generation)).toEqual([1, 2])
