...
function fanOut(panes: AtermTerminal[], cold: boolean): { ms: number; counts: number[] } {
  const counts: number[] = []
  ...
  for (const t of panes) {
    ...
    const summary = createAtermSearchSummaryReader(t).read(QUERY, false, false, MATCHES_PER_PANE)
...
describe('federated fan-out cold/warm latency (residual 5 / 5E)', () => {
  ...
  it('warm fan-out reuses the retained index and never regresses vs cold', () => {
    ...
    if (benchEnabled) {
      const indexedMb =
        (PANE_COUNT * LINES_PER_PANE * 'pane line 0 needle-HIT end'.length) / 1024 / 1024
