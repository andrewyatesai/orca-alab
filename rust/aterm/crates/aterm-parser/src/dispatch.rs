...
impl Parser {
        ...
        ProcessByte: FnMut(&mut Self, u8, &mut S),
    {
        ...
        while !remaining.is_empty() {
            ...
            // `remaining = &remaining[n..]` then lands on the control byte,
    ...
    fn parse_and_dispatch_osc<S: ActionSink>(data: &[u8], sink: &mut S, bel_terminated: bool) {
        // Fast path: single-digit command + ';' + payload with no further
        // semicolons (the overwhelmingly common case). Avoids ArrayVec init
        // (256 bytes zeroed) and the full-buffer split scan.
