...
export class DaemonPtyAdapter implements IPtyProvider {
  ...
  constructor(opts: DaemonPtyAdapterOptions) {
    ...
    this.client.onDisconnected(() => {
      ...
      // resume died with the connection. Owe those sessions a resume on reconnect.
  ...
  async shutdown(
    ...
    if (opts.keepHistory) {
      ...
      // Why: pre-populate the wake cold-restore from the just-written checkpoint
      // so it survives the kill's own exit event (guarded by sleepRestoreSessionIds).
