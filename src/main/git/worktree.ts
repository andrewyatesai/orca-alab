...
async function readWorktreeList(
  ...
  // Why: default a timeout so one wedged `git worktree list` (e.g. OneDrive/network stall) fails fast instead of hanging every later shared scan (#9786).
  const execOptions = {
    cwd: repoPath,
    ...options,
...
export function listWorktrees(
  ...
  // Why: callers with different deadlines cannot safely share which timeout wins the scan (#9786).
...
async function refreshLocalBaseRefForWorktreeCreate(
  ...
  try {
    if (evaluation.ownerWorktreePath) {
      ...
      // in the TOCTOU window after the clean-check — fail closed (report dirty)
      // rather than letting --hard silently destroy the racing edit.
      try {
        await gitExecFileAsync(
          ['reset', '--keep', evaluation.remoteOid],
          gitExecOptions(currentOwner.path, options)
        )
      } catch {
        return {
